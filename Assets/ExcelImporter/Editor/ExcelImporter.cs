...
public class ExcelImporter : AssetPostprocessor
{
    ...
    {
        Debug.Log("Import Mission Table"); string path = BrowserHelper.OpenProject("导入任务表", BrowserHelper.EXCELFILTER);
        if (path == null) return;
        ImportTable("MissionEntityTable", path);
    }
    ...
    [MenuItem("表格/导入事件表", priority = 4)]
    static void ImportIncidentTable()
    {
        Debug.Log("Import Incident Table"); string path = BrowserHelper.OpenProject("导入任务表", BrowserHelper.EXCELFILTER);
        if (path == null) return;
        ImportTable("IncidentEntityTable", path);
    }
