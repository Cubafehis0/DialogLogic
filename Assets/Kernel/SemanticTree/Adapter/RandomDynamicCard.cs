...
namespace SemanticTree.Adapter
{
    ...
    public class RandomDynamicCard : Effect
    {
        ...
        public override void Execute()
        {
            ...
            if ((PileType.Hand & type) > 0) tmp.AddRange(Context.PlayerContext.CardController.Hand);
            if ((PileType.DrawDeck & type) > 0) tmp.AddRange(Context.PlayerContext.CardController.DrawPile);
            if ((PileType.DiscardDeck & type) > 0) tmp.AddRange(Context.PlayerContext.CardController.DiscardPile);
            MyMath.Shuffle(tmp);
