...
public class CardPlayerState : MonoBehaviour, ICardPlayerState, ICardGameListener, Ink2Unity.IPlayerStateChange
{
    ...
    {
        Character.Pressure += delta;
        Player.Pressure += delta;
        if (Player.PressureSum < Player.Pressure)
        {
            HealthChange(Player.Pressure - Player.PressureSum);
            Player.Pressure = Player.PressureSum;
            Character.Pressure = Player.PressureSum;
        }
        Debug.Log("PressureChange");
    }
    public void HealthChange(int overflowPressure)
    {
        Player.Health -= overflowPressure / 3;
