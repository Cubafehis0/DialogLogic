...
public class CharacterBarView : MonoBehaviour, ICharacterView
{
    ...
    [SerializeField]
    private Text pressureText = null;
    [SerializeField]
    ...
    public void UpdateAll()
    {
        ...
        if (healthText) healthText.text = character.Health.ToString();
        if (pressureText) pressureText.text = character.Pressure.ToString() + "/" + character.PressureSum.ToString();
        if (healthSlider) healthSlider.value = character.Health;
        if (pressureSlider) pressureSlider.value = character.Pressure;
    }
}
